    ...
    private void handleLogin() {
        ...
        new Thread(() -> {
            try {
                ...
                Platform.runLater(() -> {
                    if (authenticated) {
                        com.facialaccess.util.SessionManager.setLoggedInAdmin(username);
                        new com.facialaccess.dao.AdminActionDAO().addAdminAction(username, "LOGIN", "Administrator '" + username + "' logged in successfully.");
                        navigateToDashboard();
                    } else {
                        new com.facialaccess.dao.AdminActionDAO().addAdminAction(username, "LOGIN_FAILED", "Failed login attempt for admin username '" + username + "'.");
                        int remaining = securityService.getRemainingAttempts(username);
                        if (remaining > 0) {
                            showError("Invalid credentials. " + remaining + " attempts remaining.");
                        } else {
                            new com.facialaccess.dao.AdminActionDAO().addAdminAction(username, "LOCKOUT", "Admin account '" + username + "' locked out due to excessive failed attempts.");
