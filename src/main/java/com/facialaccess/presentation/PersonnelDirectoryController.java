...
public class PersonnelDirectoryController {
    ...
    private void handleToggleActive(Utilisateur u) {
        ...
        if (result.isPresent() && result.get() == ButtonType.OK) {
            ...
            if (success) {
                new com.facialaccess.dao.AdminActionDAO().addAdminAction(
                    com.facialaccess.util.SessionManager.getLoggedInAdmin(),
                    newStatus ? "ACTIVATE_USER" : "DEACTIVATE_USER",
                    (newStatus ? "Activated" : "Deactivated") + " user '" + u.getFullName() + "' (ID: VC-" + String.format("%04d", u.getId()) + ")."
                );
                loadData();
            } else {
        ...
        if (result.isPresent() && result.get() == ButtonType.OK) {
            ...
            if (success) {
                new com.facialaccess.dao.AdminActionDAO().addAdminAction(
                    com.facialaccess.util.SessionManager.getLoggedInAdmin(),
                    "DELETE_USER",
                    "Deleted user '" + u.getFullName() + "' (ID: VC-" + String.format("%04d", u.getId()) + ")."
                );
                loadData();
            } else {
                Alert error = new Alert(Alert.AlertType.ERROR);
